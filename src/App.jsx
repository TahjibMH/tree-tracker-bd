...
import { Sprout, MapPin, Users, Building2, Landmark, ShieldAlert, Plus, TreeDeciduous } from "lucide-react";
...
export default function App() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setStorageError(null);
      setLastSynced(new Date());
      const flaggedNote = saved.verification_status === "flagged" ? " — flagged for review (large count)" :
        saved.verification_status === "verified" ? " — verified! 🎉" : "";
      setToast(`Logged ${formatBD(count)} ${species} in ${DIVISIONS.find(d => d.key === form.division).name}${flaggedNote}`);
    } catch (err) {
            ...
            {entries.map(e => {
              ...
              return (
                <div key={e.id} style={{
                  ...
                  border: "1px solid #DCE0D2", borderRadius: 10, padding: "10px 14px",
                  ...
                  <button onClick={() => openCertModal(e)} title="Get certificate" style={{
                    flexShrink: 0, background: "#EEF4EA", border: "1px solid #C9DAC2", borderRadius: 8,
                    padding: "6px 9px", cursor: "pointer", color: "#5B8C51", fontSize: 15,
                  }}>🏅</button>
                </div>
              );
            })}
